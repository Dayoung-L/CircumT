# Asymptotical covariance matrix
...
CircumT <- function(rawdt, m = 1, mcsc = "unconstrained", type = "continuous", maxit = 1000, factr = 1e9, pgtol = 0, lmm = NULL)
{
  ...
  start.value = function(dt){
    lambda = efa(rawdt, factors = 3, n.obs = N, mtest = F)$unrotated
    ...
    if(mcsc=="unconstrained") {
      beta0 = ((1/p)*sum(lambdasim[,3]))^2; beta1 = 1 - beta0
      betas=matrix(0,1,m+1); betas[1] = beta0; betas[2] = beta1
      betas = betas/betas[2]
      par <- c(angle[-c(r)]*(pi/180),betas[-c(2)],v)
    } else {
      betas=matrix(0,1,m+1); betas[1] = (1+mcsc)/2; betas[2] = (1-mcsc)/2
      betas = betas/betas[2]
      par <- c(angle[-c(r)]*(pi/180),betas[-c(2)],v)
    }
  ...
  gradient<-
    function(par){
      ...
      R = dt; p = ncol(dt)
      ang1 = par[1:(p-1)]; ang = append(ang1,0,r-1)
